...
def StructualAnalysis(wingInfo, loads):
    ...
    mesh_deformed_left = prob["disptransfer_group.def_mesh"]
    ...
    mesh_delta_left = mesh_deformed_left - wingInfo["mesh"]
    ...
    spar_pts_deformed_left = (1 - wingInfo["fem_origin"]) * mesh_deformed_left[0, :, :] + wingInfo["fem_origin"] * mesh_deformed_left[-1, :, :]
    ...
    # Thickness
    # Tip to chord [m]
    sparThickness = prob["struct_group.thickness"] 

    # Radius
    # if Radius_cp not defined，radius wiil be dependent on t_over_c
    sparRadius = prob["struct_group.radius"]

    # Intersection
    # If all the values should be negative, otherwie the hollow spar intersects itself 
    thickness_intersects = prob["struct_group.thickness_intersects"]

    # Mass
    # Element mass
    element_mass = prob["struct_group.element_mass"]

    # Total wing mass
    # Note that in the wingInfo there is a varaible call wing_weight_ratio
    wingStructuralMass = prob["struct_group.structural_mass"]	

    # Stess
    vonmisesStress = prob["struct_group.vonmises"]

    # Failure
    # Negative -> Safe, Positive -> Fail
    failure = prob["struct_group.failure"]
